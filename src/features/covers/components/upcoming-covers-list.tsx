...
export function UpcomingCoversList({
  occurrences,
  onSelectOccurrence,
}: UpcomingCoversListProps) {
  ...
  const upcoming = occurrences
    .filter((occ) => {
      ...
      return (
        isAfter(startOfDay(occDate), today) ||
  ...
  return (
    <div className="flex flex-col h-full">
      <h3 className="text-sm font-semibold text-gray-900 mb-4 px-1 flex items-center gap-2">
        <HugeiconsIcon
          icon={Calendar02Icon}
          className={ICON_SIZES.sm + ' text-primary'}
        />
            ...
            return (
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <h4
                    className={cn(
                      'font-bold text-sm truncate group-hover:underline transition-colors',
                      colors.text,
                    )}
                  >
                    {clubName}
                  ...
                  <div
                    className={cn(
                      'h-2 w-2 rounded-full mt-1.5 shadow-sm',
                      getStatusDotColor(occ),
                    )}
                  />
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2 text-[11px] font-semibold text-gray-700">
                    <HugeiconsIcon
                      icon={Calendar02Icon}
                      className={ICON_SIZES.xs + ' opacity-70'}
                    />
                  ...
                  <div className="flex items-center gap-2 text-[11px] font-semibold text-gray-700">
                    <HugeiconsIcon
                      ...
                      className={ICON_SIZES.xs + ' opacity-70'}
                    />
                    {formatEventTime(occ.cover_rule?.start_time)} -{' '}
                    {formatEventTime(occ.cover_rule?.end_time)}
                  </div>
