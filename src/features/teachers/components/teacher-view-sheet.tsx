...
export function TeacherViewSheet({
  ...
  const initials =
    `${personDetails.first_name[0] || ''}${personDetails.last_name[0] || ''}`.toUpperCase()
  const teacherId = `#T-${teacher.id.slice(0, 4).toUpperCase()}`

  // Parse styles from comma-separated strings
  const primaryStyles =
    teacher.primary_styles
      ?.split(',')
      .map((s) => s.trim())
      .filter(Boolean) ?? []
  const secondaryStyles =
    teacher.secondary_styles
      ?.split(',')
      .map((s) => s.trim())
      .filter(Boolean) ?? []
          ...
          <div className="flex items-start gap-4">
            <div className="relative">
              <Avatar className="h-16 w-16">
                <AvatarImage
                  src={personDetails.image ?? undefined}
                  alt={fullName}
                />
                <AvatarFallback className="text-lg">{initials}</AvatarFallback>
              </Avatar>
              <div
                className={`absolute bottom-0 right-0 h-4 w-4 rounded-full border-2 border-background ${teacher.is_blocked ? 'bg-destructive' : 'bg-emerald-500'}`}
              />
            </div>
            <div className="flex-1 min-w-0 pt-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge
                  variant="outline"
                  className={`shrink-0 rounded-full px-2.5 py-0.5 text-xs font-medium border-0 ${
                    teacher.is_blocked
            ...
            {documents.length > 0 ? (
              <div className="space-y-2">
                {documents.map((doc) => (
                  <div
                    key={doc.id}
                    className="flex items-center gap-3 p-3 bg-secondary/20 rounded-lg hover:bg-secondary/30 transition-colors cursor-pointer"
                  >
