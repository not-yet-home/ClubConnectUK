...
export function CoverRulesManager({
  clubId,
  schoolId,
  rules,
  isLoading,
}: CoverRulesManagerProps) {
  const [editingRule, setEditingRule] = useState<Partial<CoverRule> | null>(
    null,
  )
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {isLoading ? (
          ...
          rules.map((rule) => (
            <div
              key={rule.id}
              className="flex flex-col sm:flex-row sm:items-center justify-between p-5 rounded-lg bg-secondary/20 hover:bg-secondary/40 transition-colors gap-4 sm:gap-0"
            >
              <div className="flex items-center gap-4">
                <div className="p-2 bg-primary/10 rounded-full shrink-0">
                  <HugeiconsIcon
                    icon={Calendar03Icon}
                    className={ICON_SIZES.md + ' text-primary'}
                  />
                </div>
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge
                      variant={
                        rule.status === 'active' ? 'outline' : 'secondary'
                      }
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="w-[95vw] max-w-md sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>
              {editingRule?.id ? 'Edit Cover Rule' : 'Add Cover Rule'}
            </DialogTitle>
            <DialogDescription>
