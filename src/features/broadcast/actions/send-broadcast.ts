...
export const sendBroadcast = createServerFn({ method: 'POST' })
  ...
  .handler(async ({ data }: { data: z.infer<typeof BroadcastSchema> }) => {
    ...
    if (teacherError || teachers.length === 0) {
      throw new Error(
        'Failed to fetch selected teachers or no valid teachers found.',
      )
    }
    ...
    if (data.coverIds && data.coverIds.length > 0) {
      ...
      if (covers) {
        coverDetailsString = covers
          .map((c: any) => {
            const date = new Date(c.meeting_date).toLocaleDateString('en-GB', {
              weekday: 'short',
              day: 'numeric',
              month: 'short',
            })
    ...
    if (broadcastError) {
      throw new Error(
        `Failed to create broadcast record: ${broadcastError.message}`,
      )
    }

    const fromEmail =
      process.env.RESEND_FROM_EMAIL || 'hello@danceandarts.online'
    ...
    const results = await Promise.allSettled(
      teachers.map(async (teacher: any) => {
        ...
        if (!person?.email)
          return { error: 'No email found', teacher_id: teacher.id }
        ...
        const personalizedMessage = data.message
          ...
          .replace(
            /{{cover_details}}/g,
            coverDetailsString || '(No covers attached)',
          )
          .replace(/\n/g, '<br>')

        const personalizedSubject = data.subject.replace(
          /{{first_name}}/g,
          person.first_name || '',
        )
      ...
      .update({
        status:
          failCount === 0
            ...
            : successCount > 0
              ? 'completed'
              : 'failed', // or 'partial' if we had that status
      })
      .eq('id', broadcastData.id)

    if (successCount === 0 && failCount > 0) {
      throw new Error(
        `Failed to send to any recipients. Check logs for details.`,
      )
    }
