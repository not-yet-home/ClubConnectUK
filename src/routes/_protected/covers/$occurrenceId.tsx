...
function CoverDetailsPage() {
  ...
  const handleDelete = async () => {
    if (!window.confirm('Are you sure you want to delete this cover session?'))
      return
      ...
      >
        <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
          ...
          <p className="text-gray-500">
            The requested cover session could not be found or you don't have
            permission to view it.
          </p>
          <Button onClick={() => navigate({ to: '/covers' })}>
            Back to Calendar
          </Button>
        </div>
      </PageLayout>
  ...
  const getStatusBadge = (s: string) => {
    switch (s) {
      case 'confirmed':
        return (
          <Badge className="bg-green-100 text-green-700 hover:bg-green-200 border-green-200 gap-1.5">
            <CheckCircle2 className="w-3.5 h-3.5" /> Confirmed
          </Badge>
        )
      case 'pending':
        return (
          <Badge className="bg-yellow-100 text-yellow-700 hover:bg-yellow-200 border-yellow-200 gap-1.5">
            <Clock className="w-3.5 h-3.5" /> Pending Response
          </Badge>
        )
      case 'declined':
        return (
          <Badge className="bg-red-100 text-red-700 hover:bg-red-200 border-red-200 gap-1.5">
            <AlertCircle className="w-3.5 h-3.5" /> Declined
          </Badge>
        )
      default:
        return (
          <Badge className="bg-gray-100 text-gray-700 hover:bg-gray-200 border-gray-200 gap-1.5">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-gray-900">
                        {format(
                          new Date(occurrence.meeting_date),
                          'EEEE, MMMM d, yyyy',
                        )}
                      </p>
                    </div>
